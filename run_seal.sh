...
# 3. LOCATE BINARIES
# =========================================================================
# Main1 might be in bin/examples or bin/example or custom location
SERVER_BIN=$(find ./bin ./build -name "main1*" -type f 2>/dev/null | head -n 1)
WORKER_BIN=$(find ./bin ./build -name "worker*" -type f 2>/dev/null | grep -v "worker_bench" | head -n 1)

if [ -z "$SERVER_BIN" ]; then
    echo "Error: Server binary (main1) not found!"
    exit 1
fi
...
echo "Found Server (SealPIR Hybrid): $SERVER_BIN"
...
# =========================================================================
echo "========================================"
echo "Step 2: Starting SealPIR Hybrid Test"
echo "========================================"
echo "Cleaning up old processes..."
# Windows/Linux cleanup compatibility
taskkill //F //IM main1.exe > /dev/null 2>&1
taskkill //F //IM worker.exe > /dev/null 2>&1
pkill -f main1 > /dev/null 2>&1
...
# Force OpenMP usage
export OMP_NUM_THREADS=152
export OMP_PROC_BIND=spread
export OMP_PLACES=threads
echo "[DEBUG] OMP_NUM_THREADS=$OMP_NUM_THREADS"

# SealPIR Main usage: <config?> <workers> <queries> ?
# Old script: $SERVER_BIN "$WORKERS" "$QUERIES"
# Let's check main.cpp usage: argv[1]=config, argv[2]=workers, argv[3]=queries
...
# Wait for Server to exit (It exits after local calc is done)
wait $SERVER_PID
EXIT_CODE=$?
echo "Server exited with code $EXIT_CODE"

# Workers should have exited too (Server calls close), but we ensure cleanup
for pid in "${WORKER_PIDS[@]}"; do
    wait $pid 2>/dev/null
done
