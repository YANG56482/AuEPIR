...
void PIRServer::preprocess_database() {
  if (!is_db_preprocessed_) {
    ...
    for (uint32_t i = 0; i < db_->size(); i++) {
        // Create thread-local evaluator to avoid contention
        Evaluator local_evaluator(*context_);
        local_evaluator.transform_to_ntt_inplace(db_->operator[](i),
...
void PIRServer::set_database(const std::shared_ptr<const uint8_t[]> &bytes,
  ...
  // Optimization: Resize immediately to allow parallel random access
  result->resize(matrix_plaintexts);
  ...
  // Parallelize the entire encoding process (Data + Padding)
  #pragma omp parallel for
  for (uint64_t i = 0; i < matrix_plaintexts; i++) {
      // Thread-local encoder
      BatchEncoder local_encoder(*context_);
      
      uint64_t offset = i * bytes_per_ptxt;

      if (offset < db_size) {
          // Case 1: Encoding actual data
          uint64_t process_bytes = bytes_per_ptxt;
          ...
          if (coefficients.size() < pir_params_.slot_count) {
              coefficients.resize(pir_params_.slot_count, 1);
          } 

          local_encoder.encode(coefficients, (*result)[i]);

      } else {
          // Case 2: Padding plaintexts (matrix padding)
          // Equivalent to original padding loop
          vector<uint64_t> padding(N, 1);
          vector_to_plaintext(padding, (*result)[i]);
      }
  }
...
PirReply PIRServer::generate_reply(PirQuery &query, uint32_t client_id) {
  ...
  for (uint32_t i = 0; i < nvec.size(); i++) {
    ...
    for (uint32_t jj = 0; jj < expanded_query.size(); jj++) {
        Evaluator local_evaluator(*context_);
        local_evaluator.transform_to_ntt_inplace(expanded_query[jj]);
    }
    ...
    if ((!is_db_preprocessed_) || i > 0) {
      ...
      for (uint32_t jj = 0; jj < cur->size(); jj++) {
        Evaluator local_evaluator(*context_);
        local_evaluator.transform_to_ntt_inplace((*cur)[jj],
                                             context_->first_parms_id());
      }
    }
    ...
    */
    ...
    // Parallelize the Matrix Logic (Heavy computation)
    // Note: evaluator operations are thread safe if objects are independent.
    // intermediateCtxts[k] is independent.
    // expanded_query is read-only.
    // cur (DB) is read-only.
    #pragma omp parallel for private(temp)
    for (uint64_t k = 0; k < product; k++) {
      Evaluator local_evaluator(*context_);

      local_evaluator.multiply_plain(expanded_query[0], (*cur)[k],
                                 intermediateCtxts[k]);

      for (uint64_t j = 1; j < n_i; j++) {
        local_evaluator.multiply_plain(expanded_query[j], (*cur)[k + j * product],
                                   temp);
        local_evaluator.add_inplace(intermediateCtxts[k],
    ...
    */
    // Parallelize inverse NTT
    #pragma omp parallel for
    for (uint32_t jj = 0; jj < intermediateCtxts.size(); jj++) {
      Evaluator local_evaluator(*context_);
      local_evaluator.transform_from_ntt_inplace(intermediateCtxts[jj]);
    }
    ...
    if (i == nvec.size() - 1) {
      ...
      // This requires restructuring loop to be parallel, or locking result vector.
      // Or map-reduce.
      // Keeping sequential for now as i < nvec.size() - 1 is recursive step.
